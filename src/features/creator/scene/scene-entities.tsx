...
import { WALL_OPENING_INSET_M, wallElementRotation, wallElementToScene, type SceneBox } from "./scene-transform";
...
export function WallMarker({ element, project, appearance }: { readonly element: GymProject["wallElements"][number]; readonly project: GymProject; readonly appearance: SceneEntityAppearance }) {
  ...
  const height = isDoor ? 2.1 : 1.05;
  const lift = isDoor ? -0.15 : 0.3;
  const outlineBox: SceneBox = {
    position: { x: 0, y: lift, z: 0.025 },
  ...
  const opening = isDoor ? <>
    <mesh position={[0, lift, 0]} castShadow>
      ...
      <meshStandardMaterial color={appearance.issue ? appearance.color : "#9a5a32"} emissive={appearance.emissive}
        emissiveIntensity={0.2} roughness={0.7} polygonOffset polygonOffsetFactor={-1} polygonOffsetUnits={-1} />
    </mesh>
    <mesh position={[-width / 2, lift, 0.025]}><boxGeometry args={[0.045, height + 0.08, 0.06]} /><meshStandardMaterial color="#78350f" /></mesh>
    <mesh position={[width / 2, lift, 0.025]}><boxGeometry args={[0.045, height + 0.08, 0.06]} /><meshStandardMaterial color="#78350f" /></mesh>
    <mesh position={[0, lift + height / 2 - 0.045, 0.025]}><boxGeometry args={[width + 0.08, 0.045, 0.06]} /><meshStandardMaterial color="#78350f" /></mesh>
    <mesh position={[width * 0.3, lift, 0.055]}><sphereGeometry args={[0.035, 12, 8]} /><meshStandardMaterial color="#fbbf24" metalness={0.7} /></mesh>
  </> : <>
    <mesh position={[0, lift, 0]}>
      <boxGeometry args={[width, height, 0.025]} />
      <meshStandardMaterial color={appearance.issue ? appearance.color : "#67e8f9"} emissive={appearance.emissive}
        emissiveIntensity={0.2} transparent opacity={0.42} roughness={0.15}
        polygonOffset polygonOffsetFactor={-1} polygonOffsetUnits={-1} />
    </mesh>
    <mesh position={[-width / 2, lift, 0.025]}><boxGeometry args={[0.035, height + 0.06, 0.055]} /><meshStandardMaterial color="#475569" metalness={0.55} /></mesh>
    <mesh position={[width / 2, lift, 0.025]}><boxGeometry args={[0.035, height + 0.06, 0.055]} /><meshStandardMaterial color="#475569" metalness={0.55} /></mesh>
    <mesh position={[0, lift + height / 2, 0.025]}><boxGeometry args={[width + 0.07, 0.035, 0.055]} /><meshStandardMaterial color="#475569" metalness={0.55} /></mesh>
    <mesh position={[0, lift - height / 2, 0.025]}><boxGeometry args={[width + 0.07, 0.035, 0.055]} /><meshStandardMaterial color="#475569" metalness={0.55} /></mesh>
  </>;
  return <group position={[position.x, position.y, position.z]} rotation={[0, rotation, 0]}>
    <group position={[0, 0, WALL_OPENING_INSET_M]}>
      {opening}
